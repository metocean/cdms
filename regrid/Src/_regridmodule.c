...
static PyObject *rgd_pressattr(PyObject *self, PyObject *args) 
{
  ...
  */
  ...
  /* ------------------------------------------------------- */
    ...
    tup=Py_BuildValue(("OO"), object_wts, object_bnds);
    Py_DECREF(object_wts);
    Py_DECREF(object_bnds);
    return tup;
}
...
/*  cycle through output zones */
    ...
    */
    ...
    tup=Py_BuildValue(("OOOOOO"), object_londx, object_lonpt, object_wtlon, object_latdx, object_latpt, object_wtlat);
    Py_DECREF(object_londx);
    Py_DECREF(object_lonpt);
    Py_DECREF(object_wtlon);
    Py_DECREF(object_latdx);
    Py_DECREF(object_latpt);
    Py_DECREF(object_wtlat);
    return tup;
} 
...
/*  cycle through output zones */
    ...
    */
    ...
    tup=Py_BuildValue(("OOO"), object_latdx, object_latpt, object_wtlat);
    Py_DECREF(object_latdx);
    Py_DECREF(object_latpt);
    Py_DECREF(object_wtlat);
    return tup;
...
*    nlati = number of input grid cell latitudes.                           *
*    nlato = number of output grid cells latitudes.                         *
*    latin(nlati) = latitude of each grid cell of input field.              *
*    lonin(nloni) = longitude of each grid cell of input field.             *
*    latout(nlato) = latitude of each grid cell of output field.            *
*    lonout(nlono) = longitude of each grid cell of output field.           *
*                                                                           *
*                                                                           *
*   Output                                                                  *
*                                                                           *
...
*    londx(nlono) = index of the last element in lonpt and wtlon that       *
*                   apply to each of the the output longitudes              *
*    lonpt(2*nlono) = array of indices pointing to input grid cells     * 
*                         that contribute to each output grid cell.         *
*    wtlon(2*nlono) = array of weights indicating how much each         *
*                         input grid cell contributes to each               *
*                         output grid cell.                                 *
*                                                                           *
*    latdx(nlato) = index of the last element in latpt and wtlat that       *
*                   apply to each of the the output latitudes               *
*    latpt(2*nlato) = array of indices pointing to input grid cells     *
*                         that contribute to each output grid cell.         *
*    wtlat(2*nlato) = array of weights indicating how much each         *
*                         input grid cell contributes to each               *
*                         output grid cell.                                 *
*                                                                           *
****************************************************************************/ 
...
static PyObject *rgd_maplinear(PyObject *self, PyObject *args) 
{
    /* ----------------------- Declarations ------------------------------------*/
    ...
    float *latin;                            /* latitude of input field */
    float *latout;                           /* latitude of output field */
    float *lonin;                            /* longitude of input field */
    float *lonout;                           /* longitude of output field */
    
    PyArrayObject *object_latin;             /* latitiude of grid cells of input field */
    PyArrayObject *object_latout;            /* latitiude of output field */
    PyArrayObject *object_lonin;             /* longitude of input field */
    PyArrayObject *object_lonout;            /* longitude of output field */
    ...
    if(!PyArg_ParseTuple(args, "iiiiOOOO", &nloni, &nlono, &nlati, &nlato,  
     &object_latin, &object_latout, &object_lonin, &object_lonout))
    {
        PyErr_SetString(PyExc_TypeError, "Pass to maplinear is wrong.\n");
        return NULL;
    }

    /* assign c pointers to data in the PyArrayOjects passed in */

    latin  = (float *)object_latin->data; 
    latout = (float *)object_latout->data; 
    lonin  = (float *)object_lonin->data; 
    lonout = (float *)object_lonout->data;
	...
	for (jo = 0; jo < nlato; ++jo) {
	  last=2*jo;
	  for (ji = j1; ji < nlati-1; ji += 1) {
	    if (latout[jo] >= latin[ji] && latout[jo] <= latin[ji+1]){
	      latpt[last] = ji;
	      latpt[last+1] = ji+1;
	      wtlat[last] = latin[ji+1]-latout[jo];
	      wtlat[last+1] = latout[jo]-latin[ji];
	      if (grido) {
		  j1=ji;
	      } else {
		  j1=0;
    ...
    /*for (jo = 0; jo < nlono; ++jo) {
        if (lonout[jo]<0){lonout[jo]+=360.;}
    }
    for (ji = 0; ji < nloni; ++ji) {
        if (lonin[ji]<0){lonin[ji]+=360.;}
    }*/
    
    if (nloni==1) {
    //Trivial case of loni==lono
      lonpt[0]=0;
      lonpt[1]=0;
      wtlon[0]=1.;
      wtlon[1]=1.;
    ...
    dx=lonin[1]-lonin[0];
    if (dx<=0) {
	printf("Input longitudes must be monotically increasing");
	exit(1);
    }
    if (abs(360.+lonin[0]-lonin[nloni-1]-dx)<0.1*dx) {
      lonwrap=1;
    }
    ...
    for (jo = 0; jo < nlono; ++jo) {
        last=2*jo;
        for (ji = j1; ji < nloni-1; ++ji) {
	    if (lonout[jo] >= lonin[ji] && lonout[jo] <= lonin[ji+1]) {
            lonpt[last] = ji;
            lonpt[last+1] = ji+1;
            wtlon[last] = lonin[ji+1]-lonout[jo];
            wtlon[last+1] = lonout[jo]-lonin[ji];
//                printf("Lonout:%f Lonpt:%i %i Weight:%f %f \n",lonout[jo],lonpt[last],lonpt[last+1],wtlon[last],wtlon[last+1]);
            if (grido) {
                j1=ji;
            } else {
                j1=0;
            ...
            londx[jo]=1;
            break;
            /*Input longitude crosses prime meridian*/
	    }else if(lonin[ji]>lonin[ji+1]){
                /*Output longitude is west of prime meridian*/
            if (lonout[jo]>180. && lonout[jo] >= lonin[ji]){
                lonpt[last] = ji;
                lonpt[last+1] = ji+1;
                wtlon[last] = lonin[ji+1]+360.-lonout[jo];
                wtlon[last+1] = lonout[jo]-lonin[ji];
            /*Output longitude is east of prime meridian*/
            }else if (lonout[jo]<180. && lonout[jo] <= lonin[ji+1]){
                lonpt[last] = ji;
                lonpt[last+1] = ji+1;
                wtlon[last] = lonin[ji+1]-lonout[jo];
                wtlon[last+1] = lonout[jo]+lonin[ji]-360.;
            }
            if (grido) {
                j1=ji;
            } else {
            ...
            londx[jo]=1;
            break;
          /*Wrapped input longitude grid starts above lonout[0]*/ 
          }else if(ji==j1 && lonwrap && lonout[jo]<lonin[j1]){
            lonpt[last] = nloni-1;
            lonpt[last+1] = j1;
            wtlon[last] = lonin[j1]-lonout[jo];
            wtlon[last+1] = lonout[jo]-lonin[nloni-1]+360.;
            londx[jo]=1;
            break;
          /*Wrapped input longitude grid ends below lonout[end]*/ 
          }else if(ji==(nloni-2) && lonwrap && lonout[jo]>lonin[ji+1]){
            lonpt[last] = ji+1;
            lonpt[last+1] = 0;
            wtlon[last] = 360.+lonin[j1]-lonout[jo];
            wtlon[last+1] = lonout[jo]-lonin[ji+1];
            londx[jo]=1;
            break;
          }
        }
...
static PyObject *rgd_rgdlinear(PyObject *self, PyObject *args) 
{
    ...
    if(!PyArg_ParseTuple(args, "iiiiiiiiiiifOOOOOOOOO", &ilong, &ilat, &itim1, &itim2, &ntim1,
                                              ...
                                              &object_latpt, &object_wtlat, &object_amskin, &object_ain, &object_aout))
    {
        PyErr_SetString(PyExc_TypeError, "Pass to rgdlinear is wrong.\n");
        return NULL;
    }
    ...
    if (flag2D) {
        ...
        for (jo = 0; jo < nlato; ++jo) {
	        ...
	        if (latdx[jo]>0 && londx[io]>0) {               /* has contributing grid points */
		    wt = (double)0.;

		    for (ji = 2*jo; ji <= 2*jo+1; ++ji) {
		        iai[ilat] = latpt[ji];
		  
		        for (ii = 2*io; ii <= 2*io+1; ++ii) {
			    iai[ilong] = lonpt[ii];
			    wt += wtlat[ji] * wtlon[ii];       /* weights for all level_times */
    ...
    /* ------------------------------------------------------- */
    tup=Py_BuildValue(("O"), object_amskout);
    Py_DECREF(object_amskout);
    return tup;
}
...
static PyObject *rgd_rgdsites(PyObject *self, PyObject *args) 
{
    ...
    int ji1, ji2, ii;                   /* loop indices used for input zones and latitudes */    
    int jo, io;                   /* loop indices used for output zones and latitudes */    
    ...
    if (flag2D) {
        ...
        for (jo = 0; jo < nlato; ++jo) {
	        ...
	        if (latdx[jo]>0 && londx[jo]>0) {               /* has contributing grid points */
                wt = (double)0.;

                for (ji1 = 2*jo; ji1 <= 2*jo+1; ++ji1) {
                    for (ji2 = 2*jo; ji2 <= 2*jo+1; ++ji2) {
                    iai[ilat] = latpt[ji1];
                    iai[ilong] = lonpt[ji2];
                    wtmp = wtlat[ji1] * wtlon[ji2];       /* weights for all level_times */
                    wt+=wtmp;
		            /* cycle through the level-time structure which is the the same
                               for  input and output and for the input zones and longitudes
                               contribution */
                    ...
                    for (k2 = 0; k2 < mtim2; ++k2) {
                        ...
                        wtmsk[kk] += wtmp * amskin[n2D];
    
                        accum[kk] += wtmp * amskin[n2D] * ain[nn];
                        //printf("wtlat,wtlon,amskin,ain =  %f,%f,%f,%f\n", wtlat[ji1],wtlon[ji2],amskin[n2D],ain[nn]);
                        }
                    }
                    }
                }
			...
			//printf("wtmsk,amskout: %f,%f\n",wtmsk[kk],aout[nn]);
	 	    ...
	 	    /*  no input grid cells will contribute */
                ...
                for (k2 = 0; k2 < mtim2; ++k2) {
                    ...
                    for (k1 = 0; k1 < mtim1; ++k1) {
                        iao[jtim1] = k1;
                        nn = iao[0] + leno[0] * (iao[1] + leno[1] * (iao[ 2] + leno[2] * iao[3] ));
                        aout[nn] = omit;
                        amskout[nn] = 0.;
                    }
                }
	    ...
	    //printf("Latdx, Londx: %d,%d",latdx[jo],londx[jo]);
            if (latdx[jo] > 0 && londx[jo] > 0) {               /* has contributing grid points */
                wt = (double)0.;
                ...
                for (ji1 = 2*jo; ji1 <= 2*jo+1; ++ji1) {
                    ...
                    for (k2 = 0; k2 < mtim2; ++k2) {
                        ...
                        for (k1 = 0; k1 < mtim1; ++k1) {
                            ...
                            kk = k2 * mtim1 + k1;
                            nn = iai[0] + leni[0] * (iai[1] + leni[1] * (iai[2] + leni[2] * iai[3] ));
                            wtmsk[kk] += wtmp * amskin[nn];
                            accum[kk] += wtmp * amskin[nn] * ain[nn];
                        }
                    }
                    }
                }
...
static PyObject *rgd_rgdmissing(PyObject *self, PyObject *args) 
{
    ...
    int nlon;                              /* number of input longitudes */
    int nlat;                              /* number of input latitudes */

    int nbuf,nnb;                             /* nearest neighbour buffer */
    float omit;                             /* missing value */
    ...
    int leno[4];                   /* vector used in striding through arrays */
    int iai[4];                    /* holds 4 indices for input point */
    int iao[4];                    /* holds 4 indices for output point */  
    ...
    if(!PyArg_ParseTuple(args, "iiiiiiiiifOOO", &ilong, &ilat, &itim1, &itim2, &nlon, &nlat, &ntim1, &ntim2, &nbuf, &omit,
                                                      &object_amskin, &object_ain, &object_aout))
    {
        PyErr_SetString(PyExc_TypeError, "Pass to rgdmissing is wrong.\n");
        return NULL;
    }
    ...
    for (jo = 0; jo < nlat; ++jo) {
      ...
      for (io = 0; io < nlon; ++io) {
        iao[ilong]=io;
        n2D = iao[ilong] + leno[ilong] * iao[ilat];
        if (amskin[n2D]==0.){
          //printf("Interpolating for %i %i\n",io,jo);
          for (k = 0; k < mtim1*mtim2; ++k) {
            wtmsk[k] = (double)0.;
            accum[k] = (double)0.;
          }
          ...
          nnb=0;
          found=0;
          while((wt==0.) && (nnb<nbuf) && (found==0)){
            nnb+=1;
            for (ji = max(0,jo-nnb); ji <= min(jo+nnb,nlat-1); ++ji) {
              iai[ilat] = ji;
              for (ii = max(0,io-nnb); ii <= min(io+nnb,nlon-1); ++ii) {
                iai[ilong] = ii;
                n2D = iai[ilong] + leno[ilong] * iai[ilat];
                if (amskin[n2D]>0.){
                  //printf("Adding data for %i %i from %i %i buffer %i\n",io,jo,ii,ji,nnb);
                  wt += amskin[n2D];       /* weights for all level_times */
                  for (k2 = 0; k2 < mtim2; ++k2) {
                    iai[jtim2] = k2;
                    for (k1 = 0; k1 < mtim1; ++k1) {
                      iai[jtim1] = k1;
                      kk = k2 * mtim1 + k1;
                      nn = iai[0] + leno[0] * (iai[1] + leno[1] * (iai[2] + leno[2] * iai[3] ));
                      wtmsk[kk] += amskin[n2D];
                      accum[kk] += amskin[n2D] * ain[nn];
                    }
                  }
                  found=1;
                }
...
static struct PyMethodDef rgd_methods[] = {
  ...
  { "maplinear",     (PyCFunction)rgd_maplinear,    1 },
  { "rgdlinear",     (PyCFunction)rgd_rgdlinear,    1 },
  { "rgdsites",      (PyCFunction)rgd_rgdsites,     1 },
  { "rgdmissing",    (PyCFunction)rgd_rgdmissing,     1 },
  {  NULL,      NULL }
};
