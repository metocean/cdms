...
class Regridder:
    ...
    def __init__(self, ingrid, outgrid, type):
        self.type=type
        
        if type=='missing' and (outgrid<>ingrid):
            raise RegridError, "For missing value interpolation input and output grids must be the same"
        ...
        if self.type=='area':
            ...
            if self.type=='sites':self.nlono=-1
            self.londx, self.lonpt, self.wtlon, self.latdx, self.latpt, self.wtlat = _regrid.maplinear( self.nloni, self.nlono, self.nlati, self.nlato,
                inlat[:].astype(numpy.float32), outlat[:].astype(numpy.float32), inlon[:].astype(numpy.float32), outlon[:].astype(numpy.float32) )
            if self.type=='sites':self.nlono=1
    ...
    def __call__(self, ar, missing=None, order=None, mask=None, returnTuple=0, mbuf=1):
        ...
        if self.type=='area':
            ...
            amskout = _regrid.rgdlinear(ilon, ilat, itim1, itim2, ntim1, ntim2, nloni, self.nlono, nlati, self.nlato, flag2D, missing, self.londx, self.lonpt, self.wtlon, self.latdx, self.latpt, self.wtlat, inmask, ar, outar)
        elif self.type=='sites':
            amskout = _regrid.rgdsites(ilon, ilat, itim1, itim2, ntim1, ntim2, nloni, self.nlono, nlati, self.nlato, flag2D, missing, self.londx, self.lonpt, self.wtlon, self.latdx, self.latpt, self.wtlat, inmask, ar, outar)
            self.outlon=cdms2.createAxis(range(0,1))
        elif self.type=='missing':
            amskout = _regrid.rgdmissing(ilon, ilat, itim1, itim2, nloni, nlati, ntim1, ntim2, mbuf, missing, inmask, ar, outar)
